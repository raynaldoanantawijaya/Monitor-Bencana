...
const refactJsonWeather = (weathers = {}) => {
  ...
  for (const key in weathers.data.forecast) {
    ...
    if (key === 'area') {
      const areaArray = Array.isArray(weathers.data.forecast.area)
        ? weathers.data.forecast.area
        : [weathers.data.forecast.area];

      const areas = areaArray.map((area) => {
        // area
        const parameterArray = Array.isArray(area.parameter)
          ? area.parameter
          : [area.parameter];

        const params = parameterArray.map((parameter) => {
          // parameter
          const timeranges = Array.isArray(parameter.timerange)
            ? parameter.timerange
            : [parameter.timerange];

          const times = timeranges.map((timeRange) => {
            // timerange
